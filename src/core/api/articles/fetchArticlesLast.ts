...
export const fetchArticlesLast = async ({
  ...
  ]{ _id, title, isAdd, commentsDisabled, reactionsDisabled, likes, dislikes, "comments": count(*[_type == "comment" && references(^._id)]), views, category->{ title, name, slug, tab->{title, slug, color, tabAction }, color}, author->{name, slug}, lead, publishedAt, body, mainImage, slug} | order(publishedAt desc) [${start}..${end}]`,
    undefined,
